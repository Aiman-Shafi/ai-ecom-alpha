...
async function fetchVideoAsBase64(videoUrl: string): Promise<{ base64: string; mimeType: string }> {
  ...
  const mimeType = res.headers.get("content-type") || guessVideoMimeType(videoUrl);
...
async function runVideoInteraction(
  ...
  // Fetch video and send as inline base64 — CDN-gated URLs are not accessible to Gemini
